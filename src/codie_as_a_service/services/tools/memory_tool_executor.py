...
class MemoryToolExecutor:
    ...
    def execute(self, agent_id: str, tool_name: str, tool_input: dict[str, Any]) -> str:
        ...
        if tool_name == "read_memory":
            ...
            content = self._memory.read_memory(agent_id=agent_id, key=key)
            return content if content else f"No memory found for key: {key}"

        elif tool_name == "write_memory":
            key = tool_input.get("key", "")
            content = tool_input.get("content", "")
            self._memory.write_memory(agent_id=agent_id, key=key, content=content)
            return f"Successfully wrote to {key}"

        elif tool_name == "list_memory_keys":
            keys = self._memory.list_memory_keys(agent_id=agent_id)
